...
const DisplayScore = (score,value) => {
  ...
  var scoreBox = $("<div>").addClass("score_box text-center light-green-color width-50 margin-left-25");
  var scoreContainer = $("<div>").addClass("score_container padding-bottom-5 padding-top-5 margin-top-5  width-100  black-background green-border-2px");
  var resetButton = $("<button>").addClass("btn score_button width-90 margin-left-5 ").text("Restart")
  resetButton.click((e)=>{
  ...
  var scoreText = $("<p>").addClass("light-green-color text-center score_text").text(score+"/10").appendTo(scoreContainer);

  scoreContainer.appendTo(".quiz-container");

  scoreBox.appendTo(scoreContainer);

  resetButton.appendTo(scoreContainer);
  window.scroll(0,findPos(resetButton));
